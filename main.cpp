...
class MySwap {
public:
	MySwap() : tst1_(0), tst2_(0.0) {} // Конструктор без параметров
	MySwap(int i, double d)
		: tst1_(i), tst2_(d) {} // Конструктор с двумя параметрами
	int get_i() { return tst1_; }    // Геттер tst1_
	double get_d() { return tst2_; } // Геттер tst2_
...
public:
	...
	T &operator[](size_t ind) { return vect_[ind]; }

	void push_back(T t1) { vect_.push_back(t1); }

	void pop_back() { vect_.pop_back(); }
...
public:
	...
	MyUniquePtr(MyUniquePtr &&mp) : ptr_(nullptr) { swap(mp); }
	...
	void swap(MyUniquePtr &mp) { std::swap(ptr_, mp.ptr_); }
